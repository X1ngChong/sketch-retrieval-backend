package com.bhui.Util.Next_TO.nanjingMESO;
...
public class nanjingMESO2_3addOrderListInNextToRelation {
    public static final String labelName = "nanjingMESO";

    private static String GroupLabel = labelName + "Group2";
    private static String BuildLabel = labelName + "Build2";
    private static String RoadLabel = labelName + "Road2";
    ...
    public static void processNextToRelations(Driver driver) {
        ...
        for (List<Integer> finalList : ids) {
            if (finalList.get(2) != 0) {
                // 查询地物数据 当geometry为null的时候考虑是否是吧组数据也填充进来了,好像还有问题但是不到？。。
                String featureQuery = "MATCH (g:" + GroupLabel + ")-[r:Contain]->(f:" + BuildLabel + ") WHERE id(g) = " + finalList.get(0) +
                        " and  f.geometry IS NOT NULL  RETURN id(g) AS groupId, id(f) AS featureId, f.geometry AS geometry, type(r) AS relationship";
                // 查询道路数据
