...
/**
 * 为组节点创建next_to关系 添加了双向关系 这里要再次去neo4j执行组的init关系
 */
public class nanjingMESO2_3addNextToRelation {
   ...
   public static final String LAYER_NAME = labelName+"Road2"; // 设置道路节点所在图层的名称
    ...
    public static void processBuildingRelationships(Session session, List<Integer> buildingIds) {
            int size = buildingIds.size();
                ...
                for (int i = 0; i < size; i++) {
                    List<Integer> groupIds = fetchInitGroupIds(session,buildingIds.get(i));//initNEAR
                    for (int groupId : groupIds) {
                        Integer group1 = buildingIds.get(i);
                        Integer group2 = groupId;
    ...
    private static List<Integer> fetchInitGroupIds(Session session,Integer startId) {
        String fetchBuildingsQuery = "MATCH (b:"+Group_LABEL+")-[:InitNear]->(m)   WHERE id(b) = "+startId+"  RETURN id(m) AS id";
