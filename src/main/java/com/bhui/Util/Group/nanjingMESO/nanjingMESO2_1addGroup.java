package com.bhui.Util.Group.nanjingMESO;
 ...
 * 添加组节点
 */
public class nanjingMESO2_1addGroup {
        ...
        try (Driver driver = GraphDatabase.driver(url, AuthTokens.basic(username, password));
            ...
            List<Integer> buildingIds = fetchBuildingIds(session);
                ...
                for (Integer buildingId : buildingIds) {
                    ...
                    //查询具有init关系的build 减少操作
                    List<Integer> initBuildingIds = fetchInitBuildIds(session,buildingId);

                    // 将非道路交叉的建筑与该组关联
                    associateBuildingsToGroup(session, initBuildingIds, buildingGroupMap, groupId, buildingId);
    ...
    private static List<Integer> fetchBuildingIds(Session session) {
        String fetchBuildingsQuery = "MATCH (b:"+buildingLabel+") WHERE b.ID IS NOT NULL and b.level = 1 RETURN id(b) AS id";
    ...
    private static List<Integer> fetchInitBuildIds(Session session,Integer startId) {//对m进行限制
        String fetchBuildingsQuery = "MATCH (b:"+buildingLabel+")-[:InitNear]->(m)   WHERE id(b) = "+startId+" and m.ID IS NOT NULL and m.level = 1  RETURN id(m) AS id";
        List<Integer> buildingIds = new ArrayList<>();
        Result result = session.run(fetchBuildingsQuery);
        while (result.hasNext()) {
            Record record = result.next();
    ...
    private static boolean areBuildingsConnected(Session session, Integer buildingId1, Integer buildingId2) {
        String cypherQuery =
                "MATCH (b1:"+buildingLabel+"), (b2:"+buildingLabel+") " +
                        ...
                        "  RETURN  DISTINCT node.OBJECTID limit 2  ";
    ...
    private static void associateBuildingsToGroup(Session session, List<Integer> buildingIds,
        ...
        String addNodeToGroupQuery =
                "MATCH (g:"+groupLabel+"), (b:"+buildingLabel+") WHERE id(g) = $groupNodeId AND id(b) = $buildingId " +
                        "CREATE (g)-[:Contain]->(b)";
